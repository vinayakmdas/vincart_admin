...
class SellerDashbordwidget extends StatelessWidget {
  const SellerDashbordwidget({super.key});
          ...
          Expanded(
            ...
            child: Container(
              ...
              child: StreamBuilder<List<int>>(
                stream: sellerProvider.getWeeklySignupStream(),
                  ...
                  children: [
                    ...
                    Expanded(
                      child: StreamBuilder<int>(
                        stream: sellerProvider.getCountStream("total"),
                        builder: (context, totalSnap) {
                          return StreamBuilder<int>(
                            stream: sellerProvider.getCountStream("active"),
                            builder: (context, approvedSnap) {
                              return StreamBuilder<int>(
                                stream: sellerProvider.getCountStream("pending"),
                                builder: (context, pendingSnap) {
                                  final total = totalSnap.data ?? 0;
                                  final approved = approvedSnap.data ?? 0;
                                  final pending = pendingSnap.data ?? 0;

                                  if (!totalSnap.hasData &&
                                      !approvedSnap.hasData &&
                                      !pendingSnap.hasData) {
                                    return const Center(
                                  ...
                                  return PieChart(
                                    PieChartData(
                                      ...
                                      sections: [
                                        ...
                                        PieChartSectionData(
                                          value: approved == 0 ? 0.01 : approved.toDouble(),
                                          title: "$approved",
                                          color: Colors.green,
                                          radius: 55,
                                          titleStyle: const TextStyle(
                    ...
                    _legendItem(Colors.blue, "Total Sellers"),
                    const SizedBox(height: 8),
                    _legendItem(Colors.green, "Approved Sellers"),
                    const SizedBox(height: 8),
                    _legendItem(Colors.orange, "Pending Sellers"),
                  ],
                ),
              ),
            ),
